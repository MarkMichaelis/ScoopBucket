...
Describe "Install $name" -Tag 'Manual','Heavy','Install' {
    ...
    It 'is registered as Visual Studio 2019' {
        $vswhere = Get-Command vswhere -ErrorAction Ignore
        if ($vswhere) {
            $installations = & vswhere.exe -products '*' -property installationPath 2>$null
            $vs2019 = $installations | Where-Object { $_ -match '2019' }
            $vs2019 | Should -Not -BeNullOrEmpty
        } else {
            Get-Program -Filter '*Visual Studio*2019*' | Should -Not -BeNullOrEmpty
        }
    }
}
