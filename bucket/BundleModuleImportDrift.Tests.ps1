...
Describe 'All bundle headers use the canonical scoop-portable module import' -Tag 'Light', 'Meta' {
    ...
    It 'finds the full shipped bundle set (guards the discovery predicate)' {
        $script:BundleFiles.Count | Should -Be 18 -Because 'the discovery predicate must match exactly the 18 production bundles -- update this count deliberately when adding or removing a bundle'
    }
